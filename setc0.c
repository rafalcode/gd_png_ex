...
void writeset_png_file(char* file_name, int w, int h, png_byte color_type, png_byte bit_depth, png_bytep *row_ptrs)
{
    /* create file */
    ...
    /* row_ptrs2 */
    png_bytep *row_ptrs2 = malloc(sizeof(png_bytep)*h);
    png_byte *row, *row2, *ptr, *ptr2;
    for (y=0; y<h; y++) {
        row = row_ptrs[y];
        row_ptrs2[y] = malloc(png_get_rowbytes(png_ptr, info_ptr));
        row2 = row_ptrs2[y];
        for (x=0; x<w; x++) {
            ptr = row+x*3;
            ptr2 = row2+x*3;
...
int main(int argc, char **argv)
{
    ...
    writeset_png_file(argv[2], w, h, color_type, bit_depth, row_ptrs);
